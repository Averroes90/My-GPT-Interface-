  ...
  actions: {
    ...
    async deleteSelectedConversations(context, payload) {
      try {
        ...
        if (result && result.success) {
          context.commit('ADD_NOTIFICATION', {
            message: 'Succesfully deleted Conversation!',
            type: 'success-message',
          });
          for (const chatWindow of context.state.chatWindows) {
            await context.dispatch(
              `chat_${chatWindow.id}/populateConversationTitleList`,
              chatWindow.id
            );
          }
        } else {
    ...
    async refreshAllChatWindows(context) {
      for (const chatWindow of context.state.chatWindows) {
        await context.dispatch(`chat_${chatWindow.id}/refreshChatWindow`, {
          modelName: chatWindow.modelType,
        });
      }
    },
