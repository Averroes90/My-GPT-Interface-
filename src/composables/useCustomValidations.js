...
export function useTokenValidation(maxtokens, mintokens, maxAllowedTokens) {
    ...
    const { addNotification } = useNotifications();
      if (!/^\s*\d+\s*$/.test(maxtokens.value)) {
        addNotification('Max tokens must be a whole number without letters or special characters.', 'error-message');
        return false;
      }
  ...
  export function useConversationValidation(newConversationCheckboxState, newConversationTitle) {
    ...
    const trimmedTitle = newConversationCheckboxState.value ? newConversationTitle.value.trim() : '';
      ...
      if (newConversationCheckboxState.value && trimmedTitle === '') {
        const errorMessage = 'New Conversation Title cannot be empty when "New Conversation" is checked.'
        addNotification(errorMessage, 'error-message');
        return false;
      }
      return true;
