...
def truncate_interactions(interactions, current_model_name, token_limit, reserved_tokens):
    context_data = []
    total_tokens = 0
    ...
    for idx, interaction in enumerate(reversed(interactions)):
        ...
        if total_tokens + prompt_tokens + response_tokens <= remaining_tokens:
    ...
    return list(reversed(context_data)), truncation_index
