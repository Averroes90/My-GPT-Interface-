from models.models import Interaction, db, Conversation
...
def store_interaction(conversation_session_id, prompt, response, model_name):
    ...
    response_token_count = count_tokens(response, model_name)
    ...
    db.session.commit()
    return (
        interaction.interaction_session_id
...
def get_conversation_titles():
    ...
    latest_interaction = Interaction.query.order_by(
        Interaction.timestamp.desc()
    ).first()

    # Get the associated conversation_session_id or None if no conversations are available
    selected_conversation_id = (
        latest_interaction.conversation_session_id if latest_interaction else None
    )

    titles = [
        {"conversation_session_id": conv.conversation_session_id, "title": conv.title}
...
def conversation_title_exists(title):
    ...
    return (
        Conversation.query.filter(Conversation.title.ilike(title)).first() is not None
    )
    ...
    interactions_data = [
        {
            ...
            "interactionSessionId": interaction.interaction_session_id,
        }
...
def delete_interactions(ids):
    try:
        ...
        retrieved_ids = set()
        retrieved_ids = {
            interaction["interaction_session_id"] for interaction in interactions
        }
        ...
        try:
            Interaction.query.filter(
                Interaction.interaction_session_id.in_(ids)
...
def delete_conversations(ids):
    try:
        ...
        Interaction.query.filter(Interaction.conversation_session_id.in_(ids)).delete(
            synchronize_session="fetch"
        )

        # Then, delete the conversations themselves
        Conversation.query.filter(Conversation.conversation_session_id.in_(ids)).delete(
            synchronize_session="fetch"
        )
...
def get_interactions_by_ids(interaction_ids):
    ...
    interactions = Interaction.query.filter(
        Interaction.interaction_session_id.in_(interaction_ids)
    ).all()
