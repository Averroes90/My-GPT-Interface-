...
def fetch_interactions(conversation_session_id):
    ...
    role_tagged_interactions = add_role_tag_token_counts(
        interactions, user_role_tokens, assistant_role_tokens
    )
    # Calculate the sum of total_tokens for all interactions
    total_tokens_sum = sum(
        interaction["token_count"] for interaction in role_tagged_interactions
    )
    total_response_tokens = sum(
        interaction["response_token_count"] for interaction in role_tagged_interactions
    )
    print(f"total response tokens........ {total_response_tokens}")
    # Return the interactions and the sum of total_tokens
    return jsonify(
        {"interactions": role_tagged_interactions, "total_tokens": total_tokens_sum}
    )
...
def delete_items(itemType):
    ...
    if itemType == "interactions":
        success = delete_interactions(
            itemIds
        )  # Implement this function in db_operations.py
    elif itemType == "conversations":
        success = delete_conversations(
            itemIds
        )  # Implement this function in db_operations.py
    else:
...
def handle_prompt():
    ...
    model_name = data.get("model_name")  # Get the model name from request data
    conversation_session_id = data.get("conversation_session_id")
    new_conversation_title = data.get("new_conversation_title")
    new_conversation_checkbox = data.get("new_conversation_checkbox")
    token_limit = data.get("token_limit")  # total token limits for the model
    token_reserve = data.get(
        "token_reserve"
    )  # tokens reserved for new user's prompt in %
    ...
    user_role_tokens = count_tokens('"role": "user", "content":', model_name)
    assistant_role_tokens = count_tokens('"role": "assistant", "content":', model_name)
    prompt_token_count = count_tokens(prompt, model_name) + user_role_tokens
    if new_conversation_checkbox:
        new_conversation_title = (
            new_conversation_title.strip() if new_conversation_title else None
        )  # trim whitespaces before/after
        if not conversation_title_exists(new_conversation_title):
            ...
            response = asyncio.run(
                generate_text(message, model_name, reserved_tokens)
            )  # Only receiving the response text
            conversation_session_id = create_new_conversation(new_conversation_title)
            response_token_count = (
                count_tokens(response, model_name) + assistant_role_tokens
            )
            interactions_total_tokens = prompt_token_count + response_token_count
        else:
        ...
        interactions.append({"prompt": prompt})
        ...
        context_data, truncation_index, context_total_tokens = truncate_interactions(
            interactions,
            model_name,
            token_limit,
            reserved_tokens,
        ...
        interactions_total_tokens = context_total_tokens + response_token_count
        # Store the token counts for the latest response

    # Store the interaction in the database
    interaction_id = store_interaction(
        conversation_session_id, prompt, response, model_name
    )
    return jsonify(
        {
            "response": response,
            ...
            "context_token_count": interactions_total_tokens,
        }
    )
...
def pop_items():
    ...
    if not conversation_title_exists(new_conversation_title):
        ...
        if not interactionsIds:
            return jsonify({"success": False, "message": "No IDs provided"}), 400
        success = pop_interactions(
            interactionsIds, new_conversation_title
        )  # Implement this function in db_operations.py
