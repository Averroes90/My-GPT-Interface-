...
from db_operations import store_interaction, create_new_conversation, get_interactions_by_conversation
...
def fetch_conversation_titles():
    ...
    return jsonify({'conversation_list': titles, 
...
def fetch_interactions(conversation_session_id):
    ...
    total_tokens_sum = sum(interaction['token_count'] for interaction in role_tagged_interactions)
    total_response_tokens = sum(interaction['response_token_count'] for interaction in role_tagged_interactions)
...
def handle_prompt():
    ...
    token_reserve = data.get('token_reserve') #tokens reserved for new user's prompt in %
    ...
    if new_conversation_checkbox:
        ...
        if not conversation_title_exists(new_conversation_title):
            ...
            response_token_count = count_tokens(response, model_name) + assistant_role_tokens
    ...
    interaction_id = store_interaction(conversation_session_id, prompt, response, model_name)
    return jsonify({
        'response': response, 
