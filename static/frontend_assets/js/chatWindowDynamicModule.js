...
export const createChatWindowModule = (uniqueId) => ({
  ...
  actions: {
    async refreshChatWindow({ state, commit }, payload) {
      const response = await fetchInteractions(state.selectedConversationId , payload.modelName);
    ...
    async sendPrompt(context, payload) {
      try {
        ...
        if (payload.newConversationCheckbox) { 
          ...
          context.commit('incrementTotalTokens', data.response_tokens +data.prompt_tokens);
        }  
