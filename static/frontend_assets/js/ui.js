...
export function postResponseProcess(newConversationCheckbox, newConversationTitleElement) {
...
export function populateConversationList(titles, keepCurrentSelection, currentSelection) {
  const conversationTitleList = document.getElementById('conversationTitleList');
...
export function checkUserEntries() {
  const newConversationCheckbox = document.getElementById('newConversationCheckbox').checked;
  ...
  if (newConversationCheckbox && newConversationTitle === '') {
    alert('New Conversation Title cannot be empty when "New Conversation" is checked.');
...
export function renderTruncationPartition(truncationIndex) {
  ...
  if (truncationIndex >= 0) { // Only render if truncation occurred
    ...
    messagesContainer.insertBefore(partitionElement, messagesContainer.children[position]);
  }
}
...
export function toggleDeletionMode() {
  ...
  const conversationTitleList = document.getElementById('conversationTitleList');
  const existingDeleteInteractionsButton = document.getElementById('deleteInteractionsButton');
  const existingDeleteConversationsButton = document.getElementById('deleteConversationsButton');
...
export function refreshChatWindow() {
  ...
  api.fetchInteractions(selectedConversationSessionId)
    .then(interactions => {
...
export function createDeleteButtons() {
  ...
  deleteInteractionsButton.addEventListener('click', async function() {
    if (window.confirm('Are you sure you want to delete the selected interactions? note: selected interactions will be deleted in pairs')) {
  ...
  deleteConversationsButton.addEventListener('click', async function() {
    if (window.confirm('Are you sure you want to delete the selected conversations?')) {
